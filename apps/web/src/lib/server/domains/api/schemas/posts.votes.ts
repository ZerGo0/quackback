...
const UpdateProxyVoteBodySchema = z
  .object({
    voterPrincipalId: TypeIdSchema.meta({ description: 'Principal ID of the voter' }),
    subscriptionLevel: SubscriptionLevelSchema.meta({
...
const UpdateProxyVoteResultSchema = z
  .object({
    postId: TypeIdSchema,
...
registerPath('/posts/{postId}/vote/proxy', {
  ...
  patch: {
    ...
    requestBody: {
      required: true,
      content: { 'application/json': { schema: asSchema(UpdateProxyVoteBodySchema) } },
    },
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: createItemResponseSchema(UpdateProxyVoteResultSchema, 'Updated voter'),
          },
        },
      },
      400: {
